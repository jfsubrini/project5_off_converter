...
with the columns, primary keys, foreign keys and table engines.
...
try:
    ...
    sql = "ALTER TABLE Food ADD CONSTRAINT fk_category_id"\
        " FOREIGN KEY (category_id) REFERENCES Category(id);"\
        " ALTER TABLE Food_Store ADD CONSTRAINT fk_food_id"\
        " FOREIGN KEY (food_id) REFERENCES Food(id);"\
        " ALTER TABLE Food_Store ADD CONSTRAINT fk_store_id"\
        " FOREIGN KEY (store_id) REFERENCES Store(id);"\
        " ALTER TABLE HealthyFood ADD CONSTRAINT fk_source_id"\
        " FOREIGN KEY (source_id) REFERENCES Food(id);"\
        " ALTER TABLE HealthyFood ADD CONSTRAINT fk_substitute_id"\
        " FOREIGN KEY (substitute_id) REFERENCES Food(id)"
    cur.execute(sql)

except:
    # In case the tables have already been created.
    print("\nLes tables de la base de données offc_db ont déjà été créées.\n")

else:
    # In case everything went right with the creation of tables.
